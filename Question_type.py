...
with open('/root/autodl-tmp/VQACL/datasets/QuesId_task_map.json') as fp:
    QuesId_task_map = json.load(fp)

with open('/root/autodl-tmp/VQACL/datasets/ImgId_cate_map.json') as fp:
...
def Update_memory(M, task_idx, task):
    ...
    data_info_path = ('/root/autodl-tmp/VQACL/datasets/vqa/Partition_Q/karpathy_train_' + f'{self.task_list[task_idx - 1]}.json')
