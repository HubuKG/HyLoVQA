...
stopwords = load_file('nextqa/stopwords.txt')
...
class VQAFineTuneDataset(Dataset):
    def __init__(self, split, Examplar_set, raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train', task='DO', cates=[0,1,2]):
        ...
        sample_list_file = os.path.join('/HyLoVQA-main/datasets/nextqa/Partition_Q', '{}_{}.csv'.format(mode, task))
        ...
        # ==============================================================================
        # 🛠️ 修复版 H5 读取逻辑 (针对 feat + ids 结构)
        # ==============================================================================
        self.vid2feat = {}
        vid_feat_file = os.path.join('/HyLoVQA-main/datasets/nextqa', 'vid_feat/app_mot_{}.h5'.format(mode))
        
        print(f"📂 Loading features from: {vid_feat_file}")
        try:
            with h5py.File(vid_feat_file, 'r') as fp:
                # 一次性读取到内存 (速度快)
                all_ids = fp['ids'][:]
                all_feats = fp['feat'][:] # Shape: (N, 16, 4096)
                
                print(f"   - Loaded {len(all_ids)} videos. Processing...")
                ...
                for i, vid in enumerate(all_ids):
                    ...
                    # 2. 切片：只取 Motion (后2048维)
                    # 原始: [16, 4096] -> Motion: [16, 2048:]
                    motion_feat = all_feats[i, :, 2048:] 
                    
                    # 3. 存入字典 (转为 Tensor)
                    self.vid2feat[str(vid)] = torch.from_numpy(motion_feat).float()
    ...
    def get_video_feature(self, video_name):
        ...
        if video_name in self.vid2feat:
            return self.vid2feat[video_name] # 已经是 (16, 2048) 的 Tensor
        else:
            # ⚠️ 找不到时的兜底策略：返回全0
            # print(f"Warning: Video {video_name} not found in features!")
            return torch.zeros(16, 2048)
...
class VQAFineTuneDataset_memory(Dataset):
    def __init__(self, split, Examplar_set, raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train', task='DO', cates=[0,1,2]):
        ...
        # ==============================================================================
        # 🛠️ 修复版 H5 读取逻辑 (内存数据集也需要修复)
        # ==============================================================================
        self.vid2feat = {}
        vid_feat_file = os.path.join('/HyLoVQA-main/datasets/nextqa', 'vid_feat/app_mot_{}.h5'.format(mode))
        
        print(f"📂 Loading features for MEMORY from: {vid_feat_file}")
        try:
            with h5py.File(vid_feat_file, 'r') as fp:
                all_ids = fp['ids'][:]
                all_feats = fp['feat'][:] 
            ...
            print("✅ Memory Features loaded successfully!")
        except Exception as e:
            print(f"❌ Error loading memory features: {e}")
            self.vid2feat = {}
    ...
    def get_video_feature(self, video_name):
        ...
        video_name = str(video_name)
        if video_name in self.vid2feat:
            return self.vid2feat[video_name]
        else:
            return torch.zeros(16, 2048)
...
class VQADataset:
    ...
    def __init__(self, splits: str, verbose=True):
        ...
        sample_list_file = os.path.join('/HyLoVQA-main/datasets/nextqa', 'train.csv')
        train2014_data = load_file(sample_list_file)
        sample_list_file = os.path.join('/HyLoVQA-main/datasets/nextqa', 'val.csv')
        ...
        for split in self.splits:
            sample_list = load_file(os.path.join('/HyLoVQA-main/datasets/nextqa', split+'.csv'))
...
#         return ' '.join(words)
    ...
    def remove_stop(self, sentence):
        # === 强力修复代码 Start ===
        # 1. 如果它有 decode 方法 (bytes, numpy.bytes_, bytearray 等)，强制转码
        if hasattr(sentence, 'decode'):
            try:
                sentence = sentence.decode('utf-8')
            except Exception:
                pass # 如果解码失败，保持原样，交给后面处理
        
        # 2. 如果还不是字符串 (比如是 bytearray 但没 decode 属性)，强制转换
        if not isinstance(sentence, str):
            sentence = str(sentence)
            # 如果转出来是 "b'hello'" 这种带 b' 的字符串，去掉它 (可选，视情况而定)
            # 这里通常直接 str() 就能解决 crash 问题
        # === 强力修复代码 End ===
    ...
    def evaluate_raw(self, quesid2ans: dict, is_topk_optimal=None):
        ...
        self.evalQuesType = {}
        self.evalAnsType  = {}
        self.add_ref = load_file('/HyLoVQA-main/nextqa/add_reference_answer_test.json')
