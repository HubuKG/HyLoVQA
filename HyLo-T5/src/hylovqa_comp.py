...
# 获取当前文件的绝对路径，并添加 VQACL 目录到 sys.path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../..")))
VQACL_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
sys.path.append(VQACL_PATH)
...
def setup_logging(log_file_path):
    ...
    tqdm_output_stream = sys.__stdout__
...
class Trainer(TrainerBase):
    def __init__(self, args, coco_Ours, train_loader=None, val_loader=None, test_loader=None, train=True):
        logging.info(f"Initializing Trainer with {len(coco_Ours)} tasks")
        self.result_matrix = {}
        # [新增] 初始化 Comp 和 NonComp 矩阵，供 main_worker 使用
    ...
    def train(self, load=False):
            ...
            try:
                ...
                if args.memory:
                    if task_idx != latest_task_idx + 1:
                        ...
                        data_info_path = ('HyLoVQA-main/datasets/vqa/Partition_Q/karpathy_train_' + f'{self.task_list[task_idx - 1]}.json')
                ...
                if self.verbose:
                    ...
                    loss_meter_align = LossMeter() # [修改]: Alignment Loss Meter
                    ...
                    for epoch in range(self.args.epochs):
                        ...
                        for now_batch in now_loader:
                            ...
                            if self.verbose:
                                ...
                                # [修改]: 记录 Alignment Loss
                                if 'loss_alignment' in results and results['loss_alignment'] is not None:
                                    loss_meter_align.update(results['loss_alignment'].item())
                                    desc_str += f' | Align {loss_meter_align.val:.4f}'
                        ...
                        # [修改]: 打印 Alignment Loss
                        if len(loss_meter_align.vals) > 0:
                            align_val = loss_meter_align.val
                        else:
                            align_val = 0.0
                            
                        print("Loss:", loss_meter.val, ' Loss_mem:', loss_meter_mem.val, 'Align:', align_val)
        ...
        try:
            ...
            # [修改]: 保存 HyperNetwork
            if hasattr(self.model.module, 'hyper_generator'):
                torch.save(self.model.module.hyper_generator.state_dict(), args.output + "/HyperNetwork.pt")
                logging.info(f"Saved HyperNetwork to {args.output}")
            
            logging.info(f"Saved prototypes to {args.output}")
    ...
    def train_step(self, batch, epoch_results, task_idx, each_memory):
        ...
        # [修改]: Alignment Loss 计算
        lambda_align = getattr(self.args, 'lambda_align', 100000) 
        
        if 'loss_alignment' in results:
            loss_align = results['loss_alignment']
    ...
    def test(self, task, comp=False):
        ...
        for test_task in self.coco_Ours:
            ...
            if flag == 1:
                ...
                # [重要]: 填充 comp 和 noncomp 矩阵，供 main_worker 使用
                if test_task in Comp_task:
                    self.result_matrix_comp[task][test_task] = acc_dict_all['overall']
                else:
                    self.result_matrix_noncomp[task][test_task] = acc_dict_all['overall']
...
def main_worker(gpu, args):
    # [新增] 日志初始化
    log_file = "/HyLoVQA-main/HyLo-T5/training_comp_{}.log".format(
    ...
    args.backbone = "/HyLoVQA-main/HyLo-T5/models/t5-base"
