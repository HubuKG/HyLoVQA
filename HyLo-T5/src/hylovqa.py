...
print(f"Added {VQACL_PATH} to sys.path")
...
if version.parse(torch.__version__) < version.parse("1.6"):
    ...
    # 旧写法: from torch.cuda.amp import autocast
    # 新写法 (兼容性更好):
    try:
...
class Trainer(TrainerBase):
            ...
            try:
                ...
                for idx, cateGroup in enumerate(Category_splits_random):
                    ...
                    for epoch in range(self.args.epochs):
                        ...
                        # === [修改 3] 防崩保护：如果 loss_meter_align 是空的，显示 0 ===
    ...
    def train_step(self, batch, epoch_results, task_idx, each_memory):
        ...
        lambda_align = getattr(self.args, 'lambda_align', 10)
...
def main_worker(gpu, args):
    log_file = "/HyLoVQA-main/HyLo-T5/training_{}.log".format(
        datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    setup_logging(log_file)

    args.gpu = gpu if gpu >= 0 else 0
    args.rank = gpu
...
if __name__ == "__main__":
    ...
    if args.distributed:
        main_worker(args.local_rank if args.local_rank >= 0 else 0, args)
    else:
        main_worker(0, args)
